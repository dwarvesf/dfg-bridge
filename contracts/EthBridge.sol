...
contract EthBridge is OApp {
  ...
  function bridgeToken(
    ...
    assets[_id].safeTransferFrom(msg.sender, address(this), _amount);
  ...
  function _lzReceive(
    ...
    // need to divide by 10**18 to get the actual amount
    // since the decimals of token in src chain is 18 and dst chain is 0
    uint amount = message.amount / 10**18;
    assets[message.id].safeTransfer(message.to, amount);
